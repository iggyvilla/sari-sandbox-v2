...
- **Default to small steps: ≤ 0.5 m translation and ≤ 5–15° rotation per call.** The sandbox uses a scaled coordinate space. In open aisle space you may use up to ~1 m. If you are within ~1 m of shelves or objects drop to 0.01–0.1 m steps.
...
- Take a screenshot (`RequestScreenshot`) after significant movement or rotation to reorient before acting.
...
- After your hand leaves, a disturbed item settles. If it moved more than ~1 cm or rotated more than ~5°, it stays permanently displaced/fallen on the floor or shelf; tiny nudges are auto-restored. So a careless sweep leaves a visible mess.
...
4. Near the target, slow down, position the hand precisely, confirm `*_can_grab` is true, screenshot to check that the **correct item is highlighted in white**, then grip.
